...
class UT_Portfolio {
	...
	public function __construct( $file ) {
		...
		if ( is_admin() ) {
			...
			// Handle post columns
			add_filter( 'manage_edit-post_columns', array( &$this, 'register_post_column_headings' ), 90 , 1 );
			add_filter( 'manage_post_posts_custom_column', array( &$this, 'register_post_image_column'), 90 ,1 );
	...
	public function register_post_type() {
		...
		$labels = array(
			...
			'singular_name' => _x( 'Portfolio Item', 'post type singular name' , 'ut_portfolio_lang' ),
			'add_new' => _x( 'Add Portfolio Item', $this->token , 'ut_portfolio_lang' ),
			...
			'new_item' => sprintf( __( 'New %s' , 'ut_portfolio_lang' ), __( 'Portfolio Item' , 'ut_portfolio_lang' ) ),
			'all_items' => sprintf( __( 'All %s' , 'ut_portfolio_lang' ), __( 'Portfolio Items' , 'ut_portfolio_lang' ) ),
		...
		$args = array(
			...
			'has_archive' => UT_PORTFOLIO_ARCHIVE,
			'hierarchical' => false,
	...
	/**
	 * Helper Function to add array at specific key
	 * @since    4.8.5
	 */
	...
	public function register_post_column_headings( $defaults ) {

		$new_columns = array(
			'thumbnail' => __( 'Featured Image' , 'ut_portfolio_lang' ),
		);

		$defaults = $this->array_insert_after( $defaults, 'title', $new_columns );
	 ...
	 * @since    4.8.5
	 */
	public function register_post_image_column( $defaults ) {
		...
		if( 'thumbnail' == $defaults ) {
			...
			if( has_post_thumbnail() ) {
				...
				$mini = wp_get_attachment_image_src( get_post_thumbnail_id( $post_ID ) , 'ut-mini' );

				if( !empty( $mini[0] ) ) {

					echo '<img src="' . esc_url( $mini[0] ) . '">';
				...
				echo '<img src="' . esc_url( UT_PORTFOLIO_ASSETS_URL . 'images/placeholder-backend.jpg' ) . '">';
    ...
    public function register_custom_column_headings( $defaults ) {
        ...
        $new_columns = array(
            'thumbnail' => __( 'Featured Image' , 'ut_portfolio_lang' ),
            'portrait_thumbnail' => __( 'Featured Portrait Image' , 'ut_portfolio_lang' ),
    ...
    public function register_portfolio_category_column( $defaults ) {
        ...
        if( 'portrait_thumbnail' == $defaults ) {

            echo '<div class="ut-portfolio-post-thumbnail">';

            if( get_post_meta( $post->ID, "ut_portrait_featured_image", true ) ) {

                $mini = wp_get_attachment_image_src( get_post_meta( $post->ID, "ut_portrait_featured_image", true ) , 'ut-mini' );
