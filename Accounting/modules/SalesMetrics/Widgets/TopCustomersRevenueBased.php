...
class TopCustomersRevenueBased extends Widget
{
    ...
    public function show()
    {
        ...
        $this->applyFilters(Document::invoice(), ['date_field' => 'issued_at'])
            ->get()
            ->groupBy('contact_id')
        ...
        $this->applyFilters(Transaction::type('income')->isNotDocument()->whereNotNull('contact_id'))
            ->get()
            ->groupBy('contact_id')
            ->each(function ($contact, $key) use ($customers) {
                $amount = 0;
                ...
                $customers->put($key, collect([
                    'contact_id' => $key,
                    'contact_name' => $contact->load('contact')->first()->contact->name,
                    'amount' => $amount
                ]));
            });
